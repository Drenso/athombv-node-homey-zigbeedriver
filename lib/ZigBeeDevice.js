...
class ZigBeeDevice extends Homey.Device {
   ...
   * @param {object} args
   * @param {ZCLNode} args.zclNode
   * @param {Homey.ZigBeeNode} args.node
   * @abstract
   *
   * @example
   * const { ZigBeeDevice } = require('homey-zigbeedriver');
  ...
  async parseAttributeReport(capabilityId, cluster, payload) {
    ...
    this.setCapabilityValue(capabilityId, parsedPayload).catch(err => this.error('Error: could not set capability value, reason:', err));
  ...
  async onInit() {
    ...
    this.homey.zigbee.getNode(this)
      .then(async node => {
        ...
        this.log('ZigBeeDevice has been initialized', { firstInit: this.isFirstInit(), isSubDevice: this.isSubDevice() });
        ...
        // Legacy from homey-meshdriver
          ...
          this.error('Error: \'onMeshInit()\' failed, reason:', err);
          throw err;
        }
        ...
        // Call overridable method with initialized ZCLNode
          ...
          this.error('Error: \'onNodeInit()\' failed, reason:', err);
          throw err;
        }
  ...
  _setPollInterval(capabilityId, cluster, pollInterval) {
    ...
    if (this._pollIntervals[capabilityId] && this._pollIntervals[capabilityId][cluster.ID]) {
      this.homey.clearInterval(this._pollIntervals[capabilityId][cluster.ID]);
    }
    ...
    this._pollIntervals[capabilityId][cluster.ID] = this.homey.setInterval(() => {
