...
class ZigBeeLightDevice extends ZigBeeDevice {
   ...
   * @param {boolean} [args.supportsHueAndSaturation] - If the device does not have attribute
   * `colorCapabilities` set to `hueAndSaturation` but controlling via hue and saturation is
   * still required, set this flag to true.
   * @param {boolean} [args.supportsColorTemperature] - If the device does not have attribute
   ...
   * @param {object} args
   * @param {number} [args.hue] - Range 0 - 1
   * @param {number} [args.saturation] - Range 0 - 1
   * @param {number} [args.value] - Range 0 - 1
   * @param {object} [opts]
   * @property {number} [opts.duration]
   * @returns {Promise<any>}
   */
